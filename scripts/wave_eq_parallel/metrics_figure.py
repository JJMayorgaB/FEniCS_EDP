...
plt.ylabel('Speedup')
plt.title('Curva de Speedup para wave_eq.py')
plt.grid(True)
plt.legend()
...
plt.title('Curva de Efficiency para wave_eq.py')
plt.grid(True)

# Ajustar layout y guardar figura
plt.tight_layout()
fig_filename = os.path.join(output_dir, f"wave_eq_metrics_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
